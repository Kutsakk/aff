...
## Recommended: Composition API Alternative

In modern Vue 3 with `<script setup>`, `beforeRouteEnter` is **not available** because there is no Options API component definition object. Use `onMounted` for initial data fetching and `onBeforeRouteUpdate` for subsequent navigation:
...
// Fetch data when component mounts (replaces beforeRouteEnter)
onMounted(async () => {
  user.value = await fetchUser(route.params.id)
  loading.value = false
})

// Handle subsequent param changes (same component, different route params)
...
For data that must load BEFORE navigation completes, use a route-level `beforeEnter` guard or a global `beforeEach` guard instead.

> **Important:** In Vue Router 4, the `next()` function is deprecated in favor of return-based guards. While `next()` still works for backward compatibility, it may be removed in Vue Router 5. Use `return` values instead of `next()` calls.

## The Problem (Options API)
...
## Composition API Note

As shown in the "Recommended" section above, `beforeRouteEnter` is **not available** in `<script setup>`. Use `onMounted` + `onBeforeRouteUpdate` or route-level guards instead.
