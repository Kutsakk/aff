...
**Impact: HIGH** - The third `next()` argument in navigation guards is deprecated in Vue Router 4. It is still **supported** for backward compatibility and will not cause errors, but it is **discouraged** and may be **removed in Vue Router 5**. Using it incorrectly is one of the most common sources of bugs: calling it multiple times, forgetting to call it, or calling it conditionally without proper logic. Prefer return-based guards in all new code.
