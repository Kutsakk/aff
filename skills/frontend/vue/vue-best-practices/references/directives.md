...
If you only need `mounted`/`updated`, use the function form. Note that the function shorthand runs on **both** `mounted` and `updated` hooks -- it is equivalent to `{ mounted: fn, updated: fn }`. This means the function will be called again on every re-render of the parent component, so keep the logic idempotent or add a guard check if the operation should only happen once.

```ts
const vAutofocus = (el) => el.focus()
// Equivalent to:
// const vAutofocus = { mounted: (el) => el.focus(), updated: (el) => el.focus() }
```
