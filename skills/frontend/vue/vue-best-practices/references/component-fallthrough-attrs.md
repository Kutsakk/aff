...
> **Caveat:** Wrapping `useAttrs()` values in `computed()` does **not** make them reactive. `useAttrs()` returns a plain object that always reflects the latest values when accessed, but it is not a reactive source. A `computed` that reads from it will evaluate once on creation and will **not** re-evaluate when parent attrs change. Use `onUpdated()` for attr-driven side effects, or promote the attr to a prop if you need reactive tracking.
...
// These computed values evaluate once; they will NOT update when attrs change.
// If you need reactivity, promote these attributes to props instead.
