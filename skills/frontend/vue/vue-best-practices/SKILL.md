...
> **Nuxt note:** In Nuxt projects, ignore sections about manual router setup, manual plugin installation (`app.use()`), and SSR configuration — Nuxt handles these automatically. The **SSR / Nuxt Considerations** section at the bottom still applies. Core patterns (reactivity, SFC structure, component data flow, composables, component splitting) are universal and apply fully in Nuxt.
...
- Use `defineModel()` (Vue 3.4+) for concise two-way binding instead of manual prop + emit patterns.
- Use reactive props destructure (Vue 3.5+): destructured props from `defineProps` retain reactivity automatically.
- Use `useId()` (Vue 3.5+) to generate unique IDs for accessibility attributes (e.g. form label-input pairs).

### Template Refs

- Use `useTemplateRef()` (Vue 3.5+) for type-safe, refactoring-friendly template refs instead of matching `ref()` variable names to template `ref="..."` attributes.
...
- Built-in component `<Suspense>` for async subtree fallback boundaries -> [component-suspense](references/component-suspense.md) **Note: `<Suspense>` is still experimental as of Vue 3.5.**
- Animation: use `<Transition>` for enter/leave effects, `<TransitionGroup>` for list animations. See [Vue Transition docs](https://vuejs.org/guide/built-ins/transition).
...
When building SSR or Nuxt applications, keep the following in mind:
- Avoid browser-only APIs (`window`, `document`, `localStorage`) in setup or server-executed code paths; guard with `onMounted` or `import.meta.client` (Nuxt).
- Be aware of cross-request state pollution when using singletons (stores, global state). Always create fresh instances per request.
- Hydration mismatches are a common source of bugs; ensure server-rendered HTML matches client expectations.
