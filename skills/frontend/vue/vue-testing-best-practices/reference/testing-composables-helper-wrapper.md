...
describe('useFetch', () => {
  ...
  it('fetches data on mount', async () => {
    ...
    // WARNING: app.provide() MUST be called BEFORE app.mount()!
    // The basic withSetup helper above mounts immediately, so use the
    // enhanced version with options.provide (shown below) instead.
    const [result, testApp] = withSetup(
      () => useFetch('/api/test'),
...
> **Important:** `app.provide()` must be called **before** `app.mount()`. If you call `app.provide()` after mounting, the provided values will not be available during the component's `setup()` function, causing `inject()` calls to return `undefined`.
