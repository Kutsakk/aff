...
> **Nuxt note:** The **Plugins** and **App Configuration** sections below are for manual Vue app setup (`createApp()`, `app.use()`, `app.mount()`) and do NOT apply in Nuxt — use Nuxt plugins directory (`plugins/`) instead. All other sections (Reactivity, Computed, Watchers, Components, Templates, Composables, etc.) apply normally in Nuxt projects.
...
For Vue Router debugging, see `vue-router-best-practices`. For Pinia store debugging, see `vue-pinia-best-practices`.
...
- Imported defineProps types fail with unresolvable or complex type references (Vue 3.5 significantly improved type resolution for imported and complex types) → See [ts-defineprops-imported-types-limitations](reference/ts-defineprops-imported-types-limitations.md)
