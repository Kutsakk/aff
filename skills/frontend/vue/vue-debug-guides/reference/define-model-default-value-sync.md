...
**Solution 3 - Use `defineModel({ local: true })` (Vue 3.4+):**

Starting in Vue 3.4+, `defineModel` supports a `local: true` option. When set, the model value is kept locally in the child and only synced to the parent when explicitly changed. This allows the default value to work correctly without desync:

```html
<!-- ChildComponent.vue -->
<script setup>
// local: true means the child manages its own copy of the value.
// The default is used locally, and changes are emitted to the parent.
const model = defineModel({ default: 1, local: true })
</script>
...
This is useful when the child component should have an independent initial value that doesn't need to match the parent's ref.

**Solution 4 - Use required prop or explicit undefined handling:**
```html
<!-- ChildComponent.vue -->
<script setup>
