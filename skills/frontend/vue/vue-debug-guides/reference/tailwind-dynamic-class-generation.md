...
The `tailwind.config.js` approach shown below is for **Tailwind CSS v3** and backward compatibility:
...
**Tailwind CSS v4** uses a CSS-first configuration approach with `@theme` and `@config` directives instead of `tailwind.config.js`. Safelisting and source detection work differently:

```css
/* app.css (Tailwind v4) */
@import "tailwindcss";

/* Use @source to add additional content paths for class detection */
@source "../node_modules/my-ui-lib/src/**/*.{js,vue}";

/* Use @theme to define design tokens */
@theme {
  --color-primary: #3b82f6;
  --color-danger: #ef4444;
}

/* You can still reference a v3-style config for backward compatibility */
/* @config "./tailwind.config.js"; */
```

If migrating from v3 to v4, note that `tailwind.config.js` is optional and only needed for backward compatibility via the `@config` directive. Consult the [Tailwind CSS v4 documentation](https://tailwindcss.com/docs) for the latest configuration patterns.
